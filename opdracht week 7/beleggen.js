document.addEventListener("DOMContentLoaded", () => {
    ...
    function generateRandomPrices() {
        return {
            TechCorp: Math.random() * (150 - 100) + 100,
            BioHealth: Math.random() * (110 - 80) + 80,  
            AutoFuture: Math.random() * (200 - 120) + 120
        };
    }

    function updateCompanyOptions(prices) {
        ...
        for (const [company, price] of Object.entries(prices)) {
            ...
            option.textContent = `${company} (€${price.toFixed(2)} per aandeel)`;
            companySelect.appendChild(option);
        }
    }

    
    companyPrices = generateRandomPrices();
    updateCompanyOptions(companyPrices);
